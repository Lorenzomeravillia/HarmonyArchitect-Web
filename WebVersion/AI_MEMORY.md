...
- **Smart Jazz Density Manager**: Quando un accordo genera più di 4 voci nell'upper structure, anziché svuotare incoscientemente dalla fondamentale le triadi strette, elimina le note assecondando le regole d'arrangiamento: prima rende l'accordo *rootless*, se fallisce elimina la *Quinta Giusta*. Mantiene sempre intatte le Guide Tones (Terza e Settima).

## 2. Top-Down Semantic Indexing (Metodo Jazz)
- **Outer Shell Semantic Allocation**: L'allocazione non è casuale: definisce l'Outer Shell dell'accordo agganciando rigorosamente il Lead (Soprano) all'indice fisso `voiceIdx = 6`. E la voce più bassa dell'upper structure all'indice `voiceIdx = 3`. Se le voci sono solo 3 o 2, i "buchi" cadono matematicamente sulle voci interne (4 o 5), mantenendo sempre coerenti i profili d'azione estremi sulla grafica.
...
## 5. PWA Caching Strutturale & File Versioning (`index.html`)
- **Version Bumping Obbligatorio (v58 Corrente)**: A causa del Service Worker ostile e del caching aggressivo imposto dall'architettura PWA per massimizzare la resa offline, ogni manipolazione strutturale su file JavaScript (`main.js`, `supabase-client.js`, ecc.) **DEVE** essere accompagnata da un incremento del parametro `?v=XX` su TUTTI gli script importati in `index.html`. Attualmente il progetto si trova fisicamente alla versione **v58**.
- **Splash Screen Versioning**: In `index.html`, c'è un tag `<p style="font-family:monospace;">VERSION X.Y.Z</p>` nell'overlay di start. Deve sempre rispecchiare fedelmente l'ultimo macro-avanzamento e la build corrente (es. `1.5.0 (v58)`) in modo da accertarsi visivamente, appena aperta l'app, di non essere bloccati in una build cache obsoleta.
